...
const updateOrderListPool = (request: any, response: any) => {
    ...
    if(id == null || id == "") {
        ...
        conPool.query('UPDATE bomorder.order_list SET p_id = ($2), updated = ($3) where id = ($1)', [id, p_id, sysDate], (error: any, results: any) => {
            if (error) {
                throw error
            } else {
