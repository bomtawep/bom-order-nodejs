...
const getPrivilegePool = (request: any, response: any) => {
    const { id } = request.params
    const rawResp = (rawData: any) => {
        const {id, name, pri_num, created, updated} = rawData
        const rawDataResp = {
            id: id,
            name: name,
            pri_num: pri_num,
    ...
    if(id == null || id == "") {
        ...
        conPool.query('SELECT * FROM bomorder.privilege where id = ($1)', [id], (error: any, results: any) => {
            if (error) {
                throw error
            } else {
